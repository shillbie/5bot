...
cl.login(token="EnJUWZ1tMIscMRSYTF1b.VQTBchP8LxZ1o0Ed5rS4AW.f6SrmA9p9BUG/fEhMLmrZyDOBB7yBilM/E24jReDclg=")
cl.loginResult()

ki = LINETCR.LINE() #Zorro
ki.login(token="EnIPgmkOPv2PR8RMiop1.wOe3+tIdNxRJik2Eaf7nOq.nGK6J90lob+Tm7ltyrFjGzktc+IfFPjIuVh9bI73Mnc=")
ki.loginResult()

kk = LINETCR.LINE() #Sanji
kk.login(token="En1rSFbSGln7K6kwrwic.64p7hMq5ZbXymY+T0gu3Na.GROXSg8A7nvJs6QGU27+vU/8OMlWI/Za44fu+aGfdzY=")
kk.loginResult()

kc = LINETCR.LINE() #Ussop
kc.login(token="EnHmJn6K19RxMjpvPtkd.qcJOSgzQEpbuah+2LTD5Bq.bpWUys8fRH2DjG/XICqdsPrmTUbgOc6eVHuOQ3cX3KM=")
kc.loginResult()

ks = LINETCR.LINE() #Chooper
ks.login(token="EnS4NsCyLuLH3XLnjb8b.j0tevqX/kt6xRCOKoax7sW.GrWT54yCl2f84gV5OAYw/VG0DfZSh9XUoLbuF+JrhOc=")
ks.loginResult()

print "login success pAK"
reload(sys)
...
≫ jangan baper nanti aq cium loh ≪
              ...
              if msg.from_ in admin:
                if msg.toType == 2:
                    ...
                    if wait["lang"] == "JP":
                        ...
                        random.choice(KAC).sendText(msg.to,"Sudah Terbuka coy...")
                else:
                    if wait["lang"] == "JP":
              ...
              if msg.from_ in admin:
                if msg.toType == 2:
                    ...
                    if wait["lang"] == "JP":
                        ...
                        random.choice(KAC).sendText(msg.to,"Sudah Tertutup coy...")
                else:
                    if wait["lang"] == "JP":
                        random.choice(KAC).sendText(msg.to,"Can not be used outside the group")
                    else:
                ...
                #print wait2
                 ...
                 if msg.from_ in admin:
                    if msg.to in wait2['readPoint']:
                        ...
                        cl.sendText(msg.to, "Ketik Intip dulu Kocoy...\nBaru Ketik Ciduk\nDASAR PIKUN ♪")
#-----------------------------------------------

#-----------------------------------------------
         #----------------Fungsi Join Group Start-----------------------#
            elif msg.text in ["Kuy","melebu","Join kuy"]: #Panggil Semua Bot
              if msg.from_ in owner:
    ...
    #-------------Fungsi Leave Group Start---------------#
            ...
            elif msg.text in ["Kaboor","ngacir"]: #Semua Bot Ninggalin Group Kecuali Bot Induk
              if msg.from_ in admin:
                if msg.toType == 2:
                    ginfo = cl.getGroup(msg.to)
                    try:
    ...
    #-------------Fungsi Tag All Finish---------------#
        ...
        #----------------Fungsi Banned Kick Target Finish----------------------#                
            ...
            elif "bantai bot" in msg.text:
              if msg.from_ in owner:
                if msg.toType == 2:
                    print "ok"
                    _name = msg.text.replace("bantai bot","")
                    gs = cl.getGroup(msg.to)
                    gs = ki.getGroup(msg.to)
                    gs = kk.getGroup(msg.to)
                    gs = kc.getGroup(msg.to)
                        ...
                        if targets == []:
                            ...
                            for target in targets:
                                try:
                                    ...
                                    f=codecs.open('st2__b.json','w','utf-8')
                                    json.dump(wait["blacklist"], f, sort_keys=True, indent=4,ensure_ascii=False)
                                    random.choice(KAC).sendText(msg.to,"Succes coy...")
                                except:
...
#-----------------------------------------------
       ...
       #-------------Fungsi Respon Start---------------------#
              ...
              if msg.from_ in admin:
                ...
                ki.sendText(msg.to,"Tukang Sol On")
      ...
      #-------------Fungsi Chat ----------------
                 ...
                 quote = ['Istri yang baik itu Istri yang Mengizinkan Suaminya untuk Poligami 😂😂😂.','Kunci Untuk Bikin Suami Bahagia itu cuma satu..\nIzinkan Suamimu Untuk Selingkuh Coyyy ','brisik lu akh!','Ah Kupret Lu','Muka Lu Kaya Jamban','Ada Orang kah disini?','cocotmu mambu','Ada Perawan Nganggur ga Coy?']
...
#---------------------
        if op.type == 17:
          ...
          random.choice(KAC).sendText(op.param1, "keamanan Grup " + str(ginfo.name) + " :\n" + ginfo.creator.displayName)
          random.choice(KAC).sendText(op.param1,"Budayakan Baca Note !!! yah Ka 😊\nSemoga Betah Kk 😘")
          random.choice(KAC).sendText(op.param1,"ini FOUNDER KAMI YANG PALING BAWEL😂 \ntp baik ko😘 namanya kak WINNA")
          msg.contentMetadata = {'mid': 'u5baffcc81a0a0689982216a005cfc70b'}
        ...
        if op.type == 15:
          ...
          random.choice(KAC).sendText(op.param1, "Gara gara smalem gak di kash jatah😂😂😂")
    ...
    for zx in range(0,500):
        ...
        if hasil['result']['posts'][zx]['postInfo']['liked'] == False:
            if hasil['result']['posts'][zx]['userInfo']['mid'] in owner:
                ...
                print "Status Sudah di Like coy..."
